...
// contract disk is ERC721 { constructor () public ERC721("",""){ } } 
// ERC721 m = new ERC721("a", "b");
...
contract Diskify {
    ...
    //// Logs on function call getAlbum: register solicitation to album.
    event CallGetAlbum(address indexed _caller, uint256 _block_timestamp, uint256 _album_id_counter);
    
    //// Logs on function call getAlbumIdCounter or getPersonIdCounter: register solicitation to counter (generator-mapper) upper limit.
    event GetCounter(address indexed _owner, uint256 _block_timestamp);
	...
	function getAlbum(uint256 _album_id) public hasAlbum(msg.sender, _album_id) returns(Album memory) {
        
        emit CallGetAlbum(msg.sender, block.timestamp, _album_id);
    ...
    function getAlbumIdCounter() public isOwner returns(uint256) {
        
        emit GetCounter(msg.sender, block.timestamp);
    ...
    function getPersonIdCounter() public isOwner returns(uint256) {
